...
const generateGoodsReceiptorDelivery = (req, res, next, wave) => {
  const type = wave.includes("ECF")
    ? "goodsReceipt/processOrders"
    : "shipping/processOrders";
...
const getWaves = async (req, res, next) => {
  ...
  for (let i = 0; i < waves.length; i++) {
    const wave_employee = await employeesModel.getEmployeebyID(
      waves[i].id_employee
    );
    waves[i].employee_name = wave_employee.username;
  }
